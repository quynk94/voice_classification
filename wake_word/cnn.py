...
def train(args):
    if not op.exists('extracted_data/feat.npy') or not op.exists('extracted_data/label.npy'):
...
def predict(args):
    if op.exists(args.model):
        ...
        pred = model.predict_classes(X_predict, batch_size=16)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model', metavar='path',
                        default='models/cnn.h5', help='use this model path on train and predict operations')
    parser.add_argument('-e', '--epochs', metavar='N',
                        default=100, help='epochs to train', type=int)
    ...
    parser.add_argument('-b', '--batch-size', metavar='size',
                        default=16, help='batch size', type=int)
